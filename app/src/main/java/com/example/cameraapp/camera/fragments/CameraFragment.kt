...
class CameraFragment : Fragment() {
 ...
 /*   private val navController: NavController by lazy {
    ...
    private val characteristics: CameraCharacteristics by lazy {
        cameraManager.getCameraCharacteristics("0"/*args.cameraId*/)
    }
        ...
        fragmentCameraBinding.captureButton.setOnClickListener {
            ...
            lifecycleScope.launch(Dispatchers.IO) {
                takePhoto().use { result ->
                    ...
                    (activity as CameraActivity).imagePath=output.absolutePath
                    (activity as CameraActivity).orientation=result.orientation
                    (activity as CameraActivity).depth=Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q &&
                            result.format == ImageFormat.DEPTH_JPEG
                    (activity as CameraActivity).switchFragment(1)
                }
        ...
        when (result.format) {
            ...
            ImageFormat.JPEG, ImageFormat.DEPTH_JPEG -> {
                ...
                try {
                    ...
                    val output = createImageFile()
                    FileOutputStream(output).use { it.write(bytes) }
                    if((activity as CameraActivity).makePNGFile) {
/*                        val outputPNGFile = createFile(requireContext(), "png")
                        val outPNG: FileOutputStream = FileOutputStream(outputPNGFile)
                        val bmp: Bitmap = BitmapFactory.decodeByteArray(bytes, 0, bytes.size)
                        bmp.compress(Bitmap.CompressFormat.PNG, 80, outPNG)
                        outPNG.flush()
                        outPNG.close()*/
                    }
                    cont.resume(output)
    ...
    private fun createImageFile(): File {
        ...
        picturePath = getImgPath("IMG_${getCurrentDateTime()}.jpg")
    ...
    fun getImgPath(name: String): String {
        ...
        val sdCard: File? =activity?.applicationContext?.getExternalFilesDir(null)
        val dir = File(sdCard?.absolutePath + "/" + "images")
        if (!dir.exists()) dir.mkdirs()
        return (activity?.applicationContext?.getExternalFilesDir(null)?.getAbsolutePath().toString() + "/" + "images" + "/" + name)
    }
}
